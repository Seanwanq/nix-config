...

{
  ...
  # 盒盖和电源按钮行为配置
  services.logind = {
    # 合上盖子时强制深度挂起 (suspend-then-hibernate)
      ...
      lid = {
        ...
        action = ''
          ...
          if [ "$LID_STATE" = "closed" ]; then
            logger "ACPI: Lid closed, forcing suspend-then-hibernate..."
            # 先尝试禁用可能唤醒系统的设备
            echo disabled > /proc/acpi/wakeup/XHCI 2>/dev/null || true
            echo disabled > /proc/acpi/wakeup/CNVW 2>/dev/null || true
            # 强制挂起
            systemctl suspend-then-hibernate
          else
            logger "ACPI: Lid opened"
            # 重新启用必要的唤醒设备
            echo enabled > /proc/acpi/wakeup/LID0 2>/dev/null || true
          fi
        '';
